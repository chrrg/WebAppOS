...
public class HtmlCompressor {
    ...
    // <script></script>
    private static Pattern scriptPattern = Pattern.compile("(?:<script.*?>|<script type=['\"]text/javascript['\"]\\s*>)(.*?)</script>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
...
// 去掉注释，换行
    ...
    public static String compressJspJs(String source){
        return JSCompressor.compress(source);//CH
...
//        HtmlCompressor aa = new HtmlCompressor();
        System.out.println(compressJspJs("var a=1; \n \t a = 333;b=666;app({  \t  })"));
